/**
 * Retry discipline (R2) shared by the trust pipeline and the worker queue.
 * A job that fails *validation/judgment* is FINISHED (REJECTED) — re-running it
 * would fail the same way. Only *infrastructure* faults (network, 429, timeout,
 * malformed upstream) are worth retrying.
 */
